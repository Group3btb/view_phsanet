<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%> 

<div id="re-title​">
<div class="container">
	<span class="span  text-primary">ប្រភពមកពី</span>
</div>
</did>

<div id="resource" class="bg-success">
	<div class="container">
		<div class="row">
			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"><img
					src="/resources/views/resourceimg/amazone.jpg">
				</a>
			</div>

			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"> <img
					src="/resources/views/resourceimg/chantra.jpg">
				</a>
			</div>

			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"> <img
					src="/resources/views/resourceimg/kaymu.jpg">
				</a>
			</div>

			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"> <img
					src="/resources/views/resourceimg/khbuy.png">
				</a>
			</div>

			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"> <img
					src="/resources/views/resourceimg/khmer24.png">
				</a>
			</div>

			<div class="col-md-2 col-sm-4">
				<a href="#" class="thumbnail img"> <img
					src="/resources/views/resourceimg/nika.jpg">
				</a>
			</div>


		</div>

	</div>
</div>
<div id="footer" class="bg-primary">
	<div class="container" style="height:50px;">
		<div class="row">
			<div class="col-md-6 col-sm-6">
				<span class="footer-span font">Copyrights &#169;
					រក្សាសិទ្ធិគ្រប់យ៉ាងដោយ​ផ្សារនែត</span>
			</div>

			<div class="col-md-6 col-sm-6 text-right">
				<ul ​ class="foot-ul">
					<li><a href=""><i class="fa fa-volume-control-phone"></i>
							ទំនាក់ទំនង</a></li>
					<li>|</li>
					<li><a href=""><i class="fa fa-users"></i> អំពីយើង</a></li>
				</ul>
			</div>

		</div>
	</div>
</div>

<div id="scrolltop">
	<a href=""><img src="/resources/views/logo/back-to-top.png" /></a>
</div>